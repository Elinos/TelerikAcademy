...
require(['chance', 'underscore', 'student', 'animal', 'book', 'person'], function(Chance, _, Student, Animal, Book, Person) {
  var AUTHORS_NAMES, NAME_OF_SPECIES, NUMBER_OF_LEGS, animal, animals, animalsGroupedBySpeciesAndSortedByLegs, book, books, byFirstName, byLastName, chance, filteredAndSortedByLastName, filteredByAge, generateMarks, groupAndSortPeopleBy, makeAnimal, makeBook, makePerson, makeStudent, mostPopularAuthor, people, person, student, studentWithHighestMarks, students, totalNumberOfLegs;
  ...
  mostPopularAuthor = _.chain(books).groupBy('author').max(function(group) {
    return group.length;
  }).first().value().author;
  ...
  groupAndSortPeopleBy = function(property) {
    ...
    biggestGroup = _.chain(people).groupBy(property).max(function(group) {
      return group.length;
    }).value();
  ...
  console.log("The most popular first name is " + byFirstName.mostCommonProperty + " - count: " + byFirstName.count);
  return console.log("The most popular last name is " + byLastName.mostCommonProperty + " - count: " + byLastName.count);
});
